    ...
    <div class="card-custom detalhe-card">
      @if (loading()) {
        ...
        <div class="detalhe-tabs">
          ...
          <button class="detalhe-tab" [class.active]="viewMode()==='historico'" [disabled]="!dAtual()?.['editado']" (click)="selecionarHistorico()">
            Histórico
...
export class OperacaoDetalheComponent implements OnInit {
  ...
  selecionarAtual(): void { this.viewMode.set('atual'); }
  ...
  private carregarVersoes(): void {
    const id = this.dAtual()?.['id'];
    if (!id) return;
    this.loadingVersoes.set(true);
    this.api.get<any>('/api/admin/operacao/historico', { entrada_id: +id }).subscribe({
      next: (res: any) => {
  ...
  selecionarVersao(v: VersaoMeta): void {
    ...
    this.api.get<any>('/api/admin/operacao/historico/versao', { historico_id: v.historico_id }).subscribe({
      next: (res: any) => { this.dVersao.set(res?.data ?? res); },
      error: () => { this.dVersao.set(null); },
    });
  }
