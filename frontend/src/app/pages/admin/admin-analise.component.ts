...
export class AdminAnaliseComponent implements OnInit {
  ...
  private carregarLista(): void {
    this.http.get<DashboardCard[]>(`${environment.apiBaseUrl}/api/admin/metabase/dashboards`, {
      ...
      error: e => {
        this.carregandoLista.set(false);
        this.erroLista.set(this.extrairMensagem(e, 'Não foi possível carregar os dashboards.'));
      },
    });
  }
  ...
  selecionar(d: DashboardCard): void {
    ...
    this.http.get<{ url: string }>(
      ...
      next: r => {
        ...
        this.iframeUrl.set(this.sanitizer.bypassSecurityTrustResourceUrl(r.url));
      },
      error: e => {
        this.carregandoEmbed.set(false);
        this.erroEmbed.set(this.extrairMensagem(e, 'Não foi possível abrir o dashboard.'));
      },
    });
  }
